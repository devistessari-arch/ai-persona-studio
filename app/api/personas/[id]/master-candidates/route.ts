...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 try{const job=await submitMasterCandidates(prompt,`${process.env.APP_URL}/api/webhooks/fal?generationId=${generation.id}&kind=master-candidates`);await db.generation.update({where:{id:generation.id},data:{status:"PROCESSING"}});return NextResponse.json({generationId:generation.id,requestId:job.requestId},{status:202})}
