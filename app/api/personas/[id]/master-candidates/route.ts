...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const generation = await db.generation.create({
    data: { personaId: id, type: "IMAGE", provider: "fal.ai", model: "fal-ai/flux-2", prompt, finalPrompt: prompt, referenceImageIds: [] }
  });

  try {
    const job = await submitMasterCandidates(prompt, `${process.env.APP_URL}/api/webhooks/fal?generationId=${generation.id}&kind=master-candidates`);
    ...
    return NextResponse.json({ error: "Unable to submit generation" }, { status: 500 });
  }
}
